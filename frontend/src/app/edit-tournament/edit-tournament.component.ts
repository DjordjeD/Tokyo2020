...
export class EditTournamentComponent implements OnInit {
        ...
        if (competitors) {
          for (let index = 0; index < competitors.length; index++) {
            for (let j = 0; j < competitors[index].competesIn.length; j++) {
              if (
                competitors[index].competesIn[j].disciplineName ==
                this.currentTournament.disciplineName
              ) {
  ...
  saveTournament() {
    ...
    if (!this.currentTournament.individual) {
      if (
        this.selectedTeamsForm.value.length == 12 ||
        this.selectedTeamsForm.value.length == 8 ||
        this.selectedTeamsForm.value.length == 4 ||
        this.selectedTeamsForm.value.length == 16
      )
        { 
          var okay=true;
          var teams:Array<Team> = this.selectedTeamsForm.value
          for(let team of teams){
            if(team.teamMembers.length< team.teamMembers[0].competesIn[0].min )
            {
                this.error= "Add more players to the team";
                okay=false;
            } else if ( team.teamMembers.length> team.teamMembers[0].competesIn[0].max )
            {
              this.error= "Make a new team";
              okay=false;
            }
          }
          if(okay)
